...
// Character & Text Parallax
document.addEventListener('mousemove', (e) => {
    const moveX = (e.clientX - window.innerWidth / 2) * 0.01;
    const moveY = (e.clientY - window.innerHeight / 2) * 0.01;

    // Character moves slightly while maintaining center position
    ...
    // Text moves in opposite direction for depth while staying centered
...
// Marquee Animation (CSS handles the loop, but we can add interaction)
const marqueeSpans = document.querySelectorAll('.marquee-content span');
marqueeSpans.forEach(span => {
    span.addEventListener('mouseenter', () => {
        gsap.to(span, { scale: 1.1, color: '#FFD700', duration: 0.3 });
    });
    span.addEventListener('mouseleave', () => {
        gsap.to(span, { scale: 1, color: 'rgba(255, 255, 255, 0.3)', duration: 0.3 });
    });
});
